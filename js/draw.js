...
function loadData() {
    ...
    d3.csv("data/ted_clean.csv", function (d) {       
            ...
            d3.select('#list').html("");
            console.log(selectValue);
            d3.select('#list').append('p').text(function() { return getTopTalks(selectValue, data); });
...
d3.select('.select') //update list on change of category
  .on('change', function() {
    ...
    d3.select('#list').html("");
    console.log(selectValue);
    d3.select('#list').append('p').text(function() { console.log(getTopTalks(selectValue, data)); return getTopTalks(selectValue, data); });
});
